 ...
 *
 *  لا يوجد وضع تجريبي — لا يوجد تسجيل دخول محلي — Supabase فقط
 */
  ...
  async syncSessionFromSupabase(session) {
    ...
    if (profileData) {
      store.setUserProfile({
        ...
        goal: profileData.fitness_goal ?? 'fat_loss',
        activityLevel: profileData.activity_level ?? 'light',
        workoutDaysCount: profileData.training_days_per_week ?? 0,
        equipment: profileData.equipment,
        injuries: profileData.injuries,
        allergens: profileData.allergies,
        likedFoods: profileData.liked_foods,
        dislikedFoods: profileData.disliked_foods,
        targetCalories: profileData.target_calories ?? 0,
        targetProtein: profileData.target_protein ?? 0,
        targetCarbs: profileData.target_carbs ?? 0,
        targetFats: profileData.target_fats ?? 0,
        targetWaterLiters: profileData.target_water_liters ?? 0,
        targetGlasses: profileData.target_glasses ?? 0,
        onboardingCompleted: onboardingCompleted,
