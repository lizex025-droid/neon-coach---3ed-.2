    ...
    <div class="auth-page-container">
      ...
      <div class="auth-card">
        ...
        <div class="social-auth-group">
          ...
          <!-- زر Facebook -->
          <button type="button" id="facebook-auth-btn" class="btn-social btn-facebook" title="${isLogin ? 'تسجيل الدخول بحساب Facebook' : 'إنشاء حساب عبر Facebook'}" style="border-color: rgba(24,119,242,0.3);">
...
export function bindAuthViewEvents() {
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    if (currentMode === 'signup') {
      ...
      if (res.success) {
        notificationService.showToast(`أهلاً بك يا ${res.user.name}! لنبدأ بجمع بيانات خطتك التدريبية والغذائية 📋✨`, 'success');
        setTimeout(() => {
          window.location.hash = '#questionnaire';
        }, 350);
      } else {
