...
## 4. Nova (AI personal trainer) — optional

For the secure setup, add this in Vercel → **Settings → Environment Variables**, then redeploy:

| Variable | Value |
|---|---|
| `GEMINI_API_KEY` | your Gemini API key |

Nova calls `/api/gemini`, so the key stays on the server and is never exposed in browser code.
For local Vercel dev, put the same variable in `.env.local`.
